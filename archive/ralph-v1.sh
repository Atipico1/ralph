...
if dry; then
    ...
    echo "  2. E2E 검증"
    echo "     전체 유저 스토리 기반 agent-browser 테스트"
    echo "  3. 배포"
...
# agent-browser로 프로덕션 스모크 테스트
DEPLOY_VERIFY_OUTPUT=$(echo "Verify the deployed app at $APP_URL works correctly using agent-browser.

## Instructions

1. Open $APP_URL in agent-browser
2. Verify:
...
6. Output TASK_PASS when done"
    ...
    DEPLOY_VERIFY_OUTPUT=$(echo "Re-verify the deployed app at $APP_URL using agent-browser.
