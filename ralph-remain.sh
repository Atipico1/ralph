#!/bin/bash
# Ralph Remain — 남은 태스크 실행 (US-014~018)
# Usage: ./ralph-remain.sh [--dry-run] [--no-deploy] [max_retries]

set -euo pipefail

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PRD="$DIR/prd-remain.json"
...
[ -f "$PRD" ] || { echo "ERROR: prd-remain.json not found."; exit 1; }
...
ORIG_STORIES=$(jq '[.userStories[] | {id, title, acceptanceCriteria}]' "$DIR/prd.json")
...
4. Output TASK_PASS when ready for redeploy"
    ...
    DEPLOY_OUTPUT=$(echo "Verify $APP_URL with cmux-browser.
